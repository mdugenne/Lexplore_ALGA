...
path_to_network=Path(r'F:\Tercier\acquisitions') #Reset network path to local storage
...
runfiles = natsorted(list(Path(path_to_network ).expanduser().glob('*')))
# Search for mosaic files (all starting with ***collage***) in local data storage repository
mosaicfiles = dict(map(lambda file: (file.name, natsorted(list(file.rglob('collage*.png')))), runfiles))
...
df_path=pd.DataFrame(natsorted(list(mosaicfiles.keys()))).reset_index()
df_excel = pd.read_excel(path_to_network.parent / 'outputs' / 'Flowcam_recap_filtre_taille.xlsx', sheet_name='New')

for sample in list(set(natsorted(list(mosaicfiles.keys())))-set(natsorted(df_excel.Sample.unique())))[-1:]:
    df_excel = pd.read_excel(path_to_network.parent / 'outputs' / 'Flowcam_recap_filtre_taille.xlsx', sheet_name='New')
    sample_id = sample
    if len(mosaicfiles[sample])==0:
        df_excel=pd.concat([df_excel,pd.DataFrame({'Sample':sample},columns=['Sample','Volume_processed_ml','Volume_imaged_ml','0 to 10 µm','10 to 20 µm','20 to 50 µm','50 to 150 µm','Total'],index=[0])],axis=0).reset_index(drop=True)
        ...
        path_to_ecotaxa = Path(str(mosaicfiles[sample][0]).replace(path_to_network.stem, 'outputs')).expanduser().parent
                    ...
                    if len(np.where(np.in1d(labelled, rect_idx[id_of_interest]).reshape(labelled.shape), image, 0)[df_properties.at[rect_idx[id_of_interest] - 1, 'slice']][ slice(1, -1, None), slice(1, -1, None)]):
                        ...
                        if nb_labels > 0:  # Otherwise, particle was entirely in the background and should be discarded
                            ...
                            if particle_id not in id_to_skip:
                                ...
                                plt.imshow(np.pad(colored_image[df_properties.at[rect_idx[id_of_interest] - 1, 'slice']][slice(1, -1, None), slice(1, -1, None)], ((25, 25), (padding, padding), (0, 0)),'constant', constant_values=float(df_metadata.at[sample, 'Background_Intensity_Mean'])),cmap='gray')
                                ...
                                save_directory = Path(str(file.parent).replace(path_to_network.stem, 'outputs')).expanduser().parent / sample
