...
for sample in natsorted(list(set(list(mosaicfiles.keys()))-set(natsorted(list(map(lambda path: path.stem,list(Path(path_to_network /'Imaging_Flowcam' / 'Flowcam data' / 'Lexplore' / 'ecotaxa' ).expanduser().glob('Flowcam_*_lexplore*'))))))))è:
                ...
                if len(np.where(np.in1d(labelled,rect_idx[id_of_interest]).reshape(labelled.shape),image,0)[df_properties.at[rect_idx[id_of_interest]-1,'slice']][slice(1, -1, None), slice(1, -1, None)]):
                    ...
                    if nb_labels>0: #Otherwise, particle was entirely in the background and should be discarded
                        ...
                        if particle_id not in id_to_skip:
                            ...
                            #plt.imshow(np.pad(contour, ((25, 25), (padding, padding)), constant_values=0), cmap='gray_r')
                            padded_image=np.pad(colored_image[df_properties.at[rect_idx[id_of_interest] - 1, 'slice']][slice(1, -1, None), slice(1, -1, None)], ((25, 25), (padding, padding), (0, 0)), 'constant', constant_values=np.nanmean(colored_image[df_properties.at[rect_idx[id_of_interest] - 1, 'slice']][slice(1, -1, None), slice(1, -1, None)][np.invert(largest_object)]))
    ...
    bar.set_description('Working on vignettes upload for run {} (%s%%)'.format(sample) % percent, refresh=True)
    upload_thumbnails_ecotaxa_project(configuration=configuration, project_id=int(cfg_metadata['ecotaxa_'+df_metadata.loc[sample,'Ecotaxa_project'].lower()+'_projectid']), source_path=str(save_directory)+'.zip')
...
for file in df_zip.loc[df_zip.sample_id.isin(df_samples.sample_id.unique())==False,0].unique():
    try:
        ...
        upload_thumbnails_ecotaxa_project(configuration=configuration, project_id=project,source_path=str(file))
    except:
...
#df_nbss.to_csv(str(save_directory.parent / 'ecotaxa_nbss_data.csv'),index=False)
#df_nbss=pd.read_csv(str(save_directory.parent / 'ecotaxa_nbss_data.csv'))
#df_nbss=pd.read_csv(str(save_directory.parent / 'ecotaxa_nbss_data.csv'))
...
plot = (ggplot(df_nbss.query('instrument=="FlowCam Macro"').assign(datetime=lambda x: pd.to_datetime(x.Sample.astype(str).str.split('_').str[4],format='%Y%m%d')).assign(tag=lambda x: np.where(x.datetime>=pd.to_datetime('2025-03-01'),'after March','Before March'))) +
        ...
        theme_paper).draw(show=False)
plot.show()
plot = (ggplot(df_nbss.assign(instrument=lambda x: np.where(x.Sample==sample,'',x.instrument))) +
        ...
        labs(x='Equivalent circular diameter ($\mu$m)',y='Normalized Biovolume Size Spectra ($\mu$m$^{3}$ mL$^{-1}$ $\mu$m$^{-3}$)', title='',fill='') +
        scale_y_log10(breaks=np.multiply( 10 ** np.arange(np.floor(np.log10(1e-03)), np.ceil(np.log10(1e+04)), step=1).reshape( int((np.ceil(np.log10(1e-03)) - np.floor(np.log10(1e+04)))), 1), np.arange(1, 10, step=1).reshape(1, 9)).flatten(),labels=lambda l: ['10$^{%s}$' % int(np.log10(v)) if (np.log10(v)) / int(np.log10(v)) == 1 else '10$^{0}$' if v == 1 else '' for v in l]) +
        scale_x_log10( breaks=np.multiply( 10 ** np.arange(np.floor(np.log10(1e+00)), np.ceil(np.log10(1e+04)), step=1).reshape( int((np.ceil(np.log10(1e+00)) - np.floor(np.log10(1e+04)))), 1), np.arange(1, 10, step=1).reshape(1, 9)).flatten(), labels=lambda l: [int(v) if ((v / (10 ** np.floor(np.log10(v)))) == 1) else '' for v in l]) +
        guides(colour=None,fill=None)+
        theme_paper).draw(show=False)
...
plot = (ggplot(df_summary) +
        ...
        scale_x_date(date_breaks='1 month',date_labels='%b',limits=[datetime.date(2024,12,1),datetime.date(2025,12,1)]) +
