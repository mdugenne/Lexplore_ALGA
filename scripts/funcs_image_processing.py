...
def create_ecotaxa_project(ecotaxa_configuration=configuration,project_config={'clone of id':14450,'title':'Lexplore_ALGA_Flowcam_micro','instrument':'FlowCam','managers':['Bas Ibelings','Irene Muscas','Mathilde Dugenne'],'project_description':'This dataset includes thumbnails generated by a FlowCam micro. Acquisitions are done on samples collected on a daily basis at the surface of Lake Geneva as part of the ALGA project (PI: Bastiaan Ibelings)'}):
    ...
    with ecotaxa_py_client.ApiClient(ecotaxa_configuration) as api_client:
        ...
        try:
            ...
            print('Project successfully created: https://ecotaxa.obs-vlfr.fr/gui/prj/{}'.format(project_id))
...
def upload_thumbnails_ecotaxa_project(ecotaxa_configuration,project_id,source_path):
    """
      ...
      Documentation available at : https://github.com/ecotaxa/ecotaxa_py_client/blob/main/docs/ProjectsApi.md#simple_import
       :param configuration: An API instance specifying the host (Ecotaxa url) and the token generated based on Ecotaxa credentials
    ...
    if len(datafiles):
        with ecotaxa_py_client.ApiClient(ecotaxa_configuration) as api_client:
            # Create an instance of the API and Import class
            api_instance_files = ecotaxa_py_client.FilesApi(api_client)

            api_instance = ecotaxa_py_client.ProjectsApi(api_client)

            try:
                # Step 1:Upload file to prepare import task
                api_response_file = api_instance_files.post_user_file(file=str(datafiles[0]),  tag='datafiles_project_{}'.format(project_id))
                import_project_req = ImportReq(source_path=api_response_file, taxo_mappings=None,skip_loaded_files=True, skip_existing_objects=True, update_mode='')
...
def generate_ecotaxa_table(df,instrument,path_to_storage=None):
    ...
    if instrument.lower()=='cytosense':
        ...
        df_ecotaxa_acquisition=pd.concat([pd.concat([df_acquisition_lexplore_flowcam_10x,*[df_acquisition_lexplore_flowcam_10x.tail(1)]*(len(df)-1)],axis=0).reset_index(drop=True),pd.DataFrame({'acq_flow_rate': ['[f]'] + list(df.sample_flow_rate.astype(float).values)})],axis=1).assign(acq_id=['[t]'] +list(df.Sample.astype(str).values))
