...
def create_ecotaxa_project(ecotaxa_configuration=configuration,project_config={'clone of id':14450,'title':'Lexplore_ALGA_Flowcam_micro','instrument':'FlowCam','managers':['Bas Ibelings','Irene Muscas','Mathilde Dugenne'],'project_description':'This dataset includes thumbnails generated by a FlowCam micro. Acquisitions are done on samples collected on a daily basis at the surface of Lake Geneva as part of the ALGA project (PI: Bastiaan Ibelings)'},update_configuration=True):
    """
    ...
    with ecotaxa_py_client.ApiClient(ecotaxa_configuration) as api_client:
        ...
        try:
            ...
            project_model = ProjectModel(projid=project_id,title=title,instrument=instrument, managers=api_response_user,init_classif_list=initial_classification_list,
                                         ...
                                         contact=api_response_user[0], highest_right='Manage', license='Copyright',
                                         status = 'Annotate', objcount = None, pctvalidated = None, pctclassified = None, classifsettings = None, classiffieldlist = '', popoverfieldlist = None,
...
def generate_ecotaxa_table(df,instrument,path_to_storage=None):
    ...
    if instrument.lower()=='cytosense':
        ...
        df_ecotaxa_acquisition=pd.concat([pd.concat([df_acquisition_lexplore_flowcam_10x,*[df_acquisition_lexplore_flowcam_10x.tail(1)]*(len(df)-1)],axis=0).reset_index(drop=True),pd.DataFrame({'acq_flow_rate': ['[f]'] + list(df.sample_flow_rate.astype(float).values),'acq_acquisition_date':['[t]']+list(df.acq_acquisition_date.astype(str).values)})],axis=1).assign(acq_id=['[t]'] +list(df.Sample.astype(str).values))
        df_ecotaxa_process=pd.concat([df_processing_lexplore_flowcam_10x,*[df_processing_lexplore_flowcam_10x.tail(1)]*(len(df)-1)],axis=0).reset_index(drop=True).assign(process_id=['[t]'] +['process_Lexplore_ALGA']*len(df))
        df_ecotaxa=pd.concat([df_ecotaxa_object,df_ecotaxa_sample,df_ecotaxa_acquisition[['acq_id']+list(df_ecotaxa_acquisition.columns[0:-1])],df_ecotaxa_process[['process_id']+list(df_ecotaxa_process.columns[0:-1])]],axis=1)
