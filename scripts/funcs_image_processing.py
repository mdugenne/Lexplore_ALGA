    ...
    if instrument.lower()=='cytosense':
        ...
        df_ecotaxa_object=pd.concat([pd.DataFrame(
            {'img_file_name': ['[t]'] + list(df.img_file_name.values),
             ...
             }),pd.concat([pd.DataFrame(dict(zip(('object_'+pd.Series([key.replace(' ','_') for key in {**dict_properties_types_colors,**dict_properties_visual_spreadsheet}.keys() if key in df.columns])).values,[value for key,value in {**dict_properties_types_colors,**dict_properties_visual_spreadsheet}.items() if key in df.columns]))),df.rename(columns=dict(zip([column for column in df.columns if column in {**dict_properties_types_colors,**dict_properties_visual_spreadsheet}.keys()],['object_'+column.replace(' ','_') for column in df.columns if column in {**dict_properties_types_colors,**dict_properties_visual_spreadsheet}.keys()])))[['object_'+column.replace(' ','_') for column in df.columns if column in {**dict_properties_types_colors,**dict_properties_visual_spreadsheet}.keys()]]],axis=0).drop(columns=['object_image_intensity','object_slice']).reset_index(drop=True)],axis=1).reset_index(drop=True)
