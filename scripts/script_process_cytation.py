...
for sample in np.arange(len(imagefile_chlfield)):
    # Grayscale or chl-only well field
    image=skimage.io.imread(imagefile_chlfield[sample],as_gray=True)
    image = skimage.io.imread(str(imagefile_chlfield[sample]).replace('chloro_', 'chloroCFW_'))[:,:,0]
    ...
    df_labels=pd.DataFrame(skimage.measure.regionprops_table(label_image,properties=['bbox']))
    #Mask scalebar and checkerboard
    label_image[np.isin(label_image,df_labels[df_labels['bbox-2']>=image.shape[0]-500].index+1)]=0
    # to make the background transparent, pass the value of `bg_label`,
    # and leave `bg_color` as `None` and `kind` as `overlay`
    image_label_overlay = label2rgb(label_image, image=image, bg_label=0)
    #plt.figure(),plt.imshow(image_label_overlay),plt.show()
    ...
    #image[-500:,:]=0 # Mask the scalebar
    #oj_image=image.copy()
    #image[image<205] = 0  # Mask the checkerboard
    ...
    mc_image[np.isin(label_image,df_labels[df_labels['bbox-2']>=image.shape[0]-500].index+1)]=[0,0,0,255]
    ...
    fill_image = sp.ndimage.binary_fill_holes(skimage.morphology.remove_small_holes(skimage.morphology.closing(fill_image, skimage.morphology.square( np.max([1, int(1)]))).astype(bool)))
    ...
    for ROI in np.arange(1,nb_labels_rois):
        ...
        cropped_image[(labelled_rois[ROI_slice]!=ROI) ] =0 # Remove pixels outside the ROI
        ...
        label_objects, nb_labels =sp.ndimage.label(sp.ndimage.binary_closing(edges)) #(skimage.morphology.dilation(edges >0.05,skimage.morphology.star(2)))
        if nb_labels>0:
            ...
            av = nfold(skimage.morphology.dilation(largest_object,skimage.morphology.star(6)), mod=2)#, center=tuple(map(lambda val: val.tolist()[0],skimage.measure.regionprops_table(largest_object.astype(bool).astype(int),properties=['centroid']).values()))
            #plt.figure(), plt.imshow(av, cmap='gray'), plt.show()
            thresh = skimage.filters.threshold_otsu(av)
            bw =  av!=0#(skimage.segmentation.clear_border(av))#sp.ndimage.binary_fill_holes#skimage.morphology.erosion(skimage.morphology.erosion(skimage.morphology.closing(av >=0.5, skimage.morphology.star((3)))))
            ...
            bw_properties=pd.DataFrame(skimage.measure.regionprops_table(skimage.morphology.dilation(bw).astype(bool).astype(int),properties=['area','area_filled'])) if ((labelled_particle!=0).any()) else pd.DataFrame({'area_filled':np.inf,'area':1},index=[0])
            if (((labelled_particle!=0).any())) & (np.round(bw_properties.loc[0,'area_filled']/bw_properties.loc[0,'area'],3)<=1.001): #

                largest_object = labelled_particle == np.argmax(np.bincount(labelled_particle.flat)[1:]) + 1
                #plt.figure(),plt.imshow(largest_object, cmap='gray'),plt.show()
                properties=skimage.measure.regionprops_table(skimage.segmentation.clear_border(av).astype(int),properties=['perimeter','equivalent_diameter_area','eccentricity','axis_major_length','axis_minor_length'],spacing=scale_pixel_per_micron) if skimage.segmentation.clear_border(av).any() else skimage.measure.regionprops_table(largest_object.astype(int),properties=['perimeter','equivalent_diameter_area','eccentricity','axis_major_length','axis_minor_length'],spacing=scale_pixel_per_micron)


                if (((properties['perimeter'])/(2*np.pi*properties['equivalent_diameter_area']/2)>1.5)[0]) & (0.1<np.round(properties['eccentricity'],1)<0.9):
                    ...
                    if len(skeleton_image[skeleton_image == True]) > 1:
                        ...
                        edges = skimage.filters.sobel(mc_image.copy()[ROI_slice][:, :, 1:])[:,:,0]
                        #plt.figure(),plt.imshow(edges),plt.show()

                        mask_DAPI = skimage.morphology.remove_small_objects(sp.ndimage.binary_fill_holes(skimage.morphology.erosion(skimage.morphology.dilation(edges > np.quantile(edges,q=0.999)))), min_size=int( np.floor(np.pi * (scale_pixel_per_micron * 5 / 2) ** 2)))
                        #plt.figure(),plt.imshow(mask_DAPI.astype(bool).astype(int)),plt.show()
                        ...
                        if (labelled_CHL ==1).any() :
                            ...
                            # find filament end points (where degree of graph nodes is 1)
                            skeleton_image = skimage.morphology.skeletonize(skimage.segmentation.clear_border(skimage.morphology.erosion(skimage.morphology.dilation(label_CHL_positive_rois.astype(bool).astype(int) & fill_ROI,skimage.morphology.star(6)),skimage.morphology.star(6))))
                            # plt.figure(),plt.imshow(skeleton_image),plt.show()
                            if len(skeleton_image[skeleton_image == True]) > 1:
                                try:
                                    skeleton = Skeleton(skeleton_image)
    ...
    dpi=np.prod(Image.open(str(imagefile_chlfield[sample]).replace('chloro_','chloroCFW_')).info['dpi'])/10
    ...
    plt.savefig(str(imagefile_chlfield[sample]).replace('chloro','ROI_chloroCFW'), dpi=dpi, transparent=True,  bbox_inches="tight",pad_inches=0)
