...
df_summary_statistics['Treatment']=pd.Categorical(pd.Series(list(df_summary_statistics.index.astype(str))).str.split('_').str[-1].replace('nofixative','No fixative').replace('glut','0.25% glutaraldehyde'))
df_summary_statistics['Timepoint']=pd.Categorical('T'+(df_summary_statistics['Analysis_time']-df_summary_statistics.loc['Flowcam_10x_lexplore_wasam_20241002_2024-10-04_nofixative','Analysis_time']).dt.days.astype(int).astype(str),categories=['T0','T2','T4','T6','T9','T11'],ordered=True)
df_summary_statistics['Abundance']=df_summary_statistics['Particles / ml'].astype(float)
...
plot = (ggplot(df_summary_statistics) +
        geom_bar(mapping=aes(x='Timepoint', y='Abundance', group='Treatment',fill='Treatment'), alpha=1,stat='identity', position = 'dodge') + #
        ...
        labs(x='Timepoint',y='Particles mL$^{-1}$', title='',colour='') +
         scale_y_continuous() +scale_fill_manual(values={'No fixative':'grey','0.25% glutaraldehyde':'black'})+
        theme_paper).draw(show=False)

plot.savefig(fname='{}/figures/Initial_test/flowcam_test_fixative.svg'.format(str(path_to_git)), dpi=300, bbox_inches='tight')
