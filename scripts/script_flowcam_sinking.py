...
path_to_runs=natsorted(list(path_to_dir.glob('Flo*')))
run=path_to_runs[6]
...
plot=(ggplot(df_properties_sample)+geom_point(mapping=aes(x='Time',y='Capture Y'),size=.001)+scale_y_reverse()).draw(show=True)
...
plot=(ggplot(df_sinking_speed)+labs(y='Number of observations',x='Sinking speed (m/d)',fill='')+geom_histogram(mapping=aes(x='value',fill='coefficient'),alpha=.5,colour='black',bins=50)+scale_fill_manual(values={'slope':'gray','slope_corrected':'blue'})+theme_plot+theme(legend_title_position='top')).draw(show=True)
...
df_summary=df_summary.astype({'cluster':str})
fig, ax = plt.subplots(nrows=2,height_ratios=[3,1])
with tqdm(desc='Generating reconstructed frames for cluster', total=len(df_summary.cluster.unique()), bar_format='{desc}{bar}', position=0, leave=True) as bar:
